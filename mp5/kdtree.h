...
class KDTree
{
	private:
		...
		/**
		 * vector which contains the Point objects (which represent points
		 * in Dim dimensional space). Note that you should **not** move
		 * points around in this vector: instead, you should modify the
		 * point_index vector, which contains indicies into this vector.
		 */
		vector< Point<Dim> > points;

		/**
		 * vector which contains indicies into the the points vector. For
		 * example, `points[point_index[i]]` gives you the `i`th point.
		 */
		...
		// Print helper functions (you don't need to worry about these)
	...
	public:
		...
		// Print functions (you don't need to worry about these)
		...
		/**
		 * Determines if Point a is smaller than Point b in a given
		 * dimension d. If there is a tie, break it with
		 * Point::operator<().
		 *
		 * For example:
		 *
		 ...
		 * @param d Dimension these points are being compared in.
		 * @return A boolean value indicating whether a is smaller than b
		 ...
		 *     cout << shouldReplace(target, currentBest3, possibleBest3) << endl; // based on operator<, this should be false
		 *
		 ...
		 *
		 * Because we don't want to modify the vector of Points passed in to
		 * the constructor, the actual work should be done on a vector of
		 * integers, which index into the Points vector. You will probably
		 * want to write a helper function which performs the median selection
		 * and partitioning on this integer vector. Maybe you can use a
		 ...
		 *  [Andrew Moore's KD-Tree
		 *  Tutorial](http://www.autonlab.org/autonweb/14665/version/2/part/5/data/moore-tutorial.pdf?branch=main&language=en).
		 *
		 * @see There is [an
		 * example](https://wiki.engr.illinois.edu/display/cs225/MP+6.1) on
		 * the KDTree spec on the wiki.
		 *
		 * @param a The point we wish to find the closest neighbor to in the
		 *  tree.
